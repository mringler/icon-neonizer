...
const gradientDrawerOptions = [
    {
        title: 'Random',
        value: GradientBuilderOption.random,
        description: 'Pick a random direction for each color gradient.',
    },
    {
        title: 'Fixed',
        value: GradientBuilderOption.fixed,
        description: 'Use the same direction for each gradient, every element gets full range.',
    },
    {
        title: 'Flat',
        value: GradientBuilderOption.flat,
        description: 'Use the same direction for each gradient, gradient spans the image rather then element so element coloring depends on position.',
    },
]
const inputConfig = useInputConfig(toRef(props, 'showHelp'), {
    description,
    tableData: {
        data: gradientDrawerOptions,
        keys: { title: 'Option', description: 'Description' },
    },
})
</script>
