...
export namespace Blacklist {
    ...
    export async function addPageToBlacklist(page: BlacklistedPage) {
        ...
        return Promise.all([storeBlacklist(blacklist), IconStorage.removeIcon(page.url)])
    }
    ...
    export async function getBlacklistEntry(url: string): Promise<BlacklistedPage | undefined> {
        ...
        return ulrBlacklist.find(
            (blacklistedPage) =>
                url.startsWith(blacklistedPage.url) && url !== blacklistedPage.replacementUrl
        )
    }
}
