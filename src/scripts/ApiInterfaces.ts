type ApiArgs<T, P extends keyof T = keyof T> = {
    [K in P]: T[K] extends (...args: infer A) => any ? A : never
}[P]
type Ret<T, P extends keyof T = keyof T> = {
    [K in P]: T[K] extends (...args: any) => infer R ? R : never
}[P]
export type ScriptsApi<T> = { [K in keyof T]: (...args: ApiArgs<T, K>) => Ret<T, K> }

export type ApiCaller<ApiInterface extends ScriptsApi<ApiInterface>> = <
    CommandName extends keyof ApiInterface
>(
...
export type ApiListener<ApiInterface extends ScriptsApi<ApiInterface>> = <
    CommandName extends keyof ApiInterface
>(
    message: {
        command: CommandName
